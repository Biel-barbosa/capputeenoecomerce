...
const Container = styled.div<{ cartItems: CartItem[] }>`
  max-width: 1440px;
  margin: 0 auto;
  padding: 0 2rem;
  display: flex;
  flex-direction: ${({ cartItems }) => (cartItems.length === 0 ? 'column' : 'row')}; /* Coluna quando vazio, linha quando tem itens */
  justify-content: ${({ cartItems }) => (cartItems.length === 0 ? 'center' : 'space-between')}; /* Alinhar ao centro quando vazio */
  align-items: ${({ cartItems }) => (cartItems.length === 0 ? 'center' : 'flex-start')}; /* Alinhar ao centro quando vazio */
  gap: 2rem;
...
export default function CartPage() {
  ...
  return (
    <PageWrapper>
      <Header />
      <Container cartItems={cart}> {/* Passando o cartItems como prop */}
        {cart.length === 0 ? (
          <EmptyCartMessage>
